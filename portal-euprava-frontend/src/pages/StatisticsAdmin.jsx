...
export default function StatisticsAdmin() {
  ...
  // OPTIONAL: Top services / by_service if backend returns it.
  ...
  // OPTIONAL: Top institutions / by_institution if backend returns it.
  const topInstitutions = useMemo(() => {
    const raw =
      (Array.isArray(stats?.top_institutions) && stats.top_institutions) ||
      (Array.isArray(stats?.by_institution) && stats.by_institution) ||
      [];
    return raw
      .map((x) => {
        const name =
              ...
              >
                <Group justify="space-between" align="flex-start">
                  <div>
                    ...
                    <Text c="dimmed" mt={4}>
                      Ukupan pregled sistema (zahtevi, statusi i agregacije).
                    </Text>
                  </div>
            ...
            >
              {loading ? (
                ...
                <Stack gap="lg">
                  ...
                  {/* Optional blocks (if backend sends these arrays) */}
                  <SimpleGrid cols={{ base: 1, md: 2 }} spacing="md">
                    ...
                    {topInstitutions.length > 0 && (
                      <Card withBorder radius="xl" p="lg">
                        ...
                        <div style={{ height: 320 }}>
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={topInstitutions} layout="vertical" margin={{ left: 30 }}>
                              <CartesianGrid strokeDasharray="3 3" />
                              <XAxis type="number" allowDecimals={false} />
                  ...
                  {(topServices.length === 0 && topInstitutions.length === 0) && (
                    <Card withBorder radius="xl" p="lg">
                      <Text c="dimmed">
                        Backend trenutno vraća samo ukupne metrike i by_status. Ako dodaš na ruti /stats polja tipa
                        <Text span fw={700}> top_services</Text> ili <Text span fw={700}> by_institution</Text>,
                        ovde će se automatski pojaviti dodatni grafici.
                      </Text>
                    </Card>
                  )}
                </Stack>
